 ...
 */
public class AceVerifyFilter extends OncePerRequestFilter {

    private final RedisTemplate<String, AceUser> userDetailRedisTemplate;
    private final ObjectMapper objectMapper;

    public AceVerifyFilter(RedisTemplate<String, AceUser> userDetailRedisTemplate, ObjectMapper objectMapper) {
    ...
    protected void doFilterInternal(HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
        ...
        if (header != null && header.startsWith("Bearer")) {
            //从token转用户
            final AceUser aceUser = userDetailRedisTemplate.opsForValue().get(String.format(SecurityConstants.USER_DETAIL_PREFIX, header.split(" ")[1]));
            if (aceUser != null) {
