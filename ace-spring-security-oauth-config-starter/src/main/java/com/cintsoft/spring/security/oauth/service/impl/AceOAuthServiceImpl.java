...
public class AceOAuthServiceImpl implements AceOAuthService {
    ...
    public String authorize(Model model, HttpServletRequest request, HttpServletResponse response, HttpSession session, AceAuthorizeParams aceAuthorizeParams) {
        ...
        if (AceOAuthConstant.GRANT_TYPE_CODE.equals(aceAuthorizeParams.responseType)) {
            ...
            response.sendRedirect(String.format(AceOAuthConstant.AUTHORIZATION_CODE_URL, clientDetails.getWebServerRedirectUri(), code, aceAuthorizeParams.state));
        } else if (AceOAuthConstant.GRANT_TYPE_TOKEN.equals(aceAuthorizeParams.responseType)) {
            response.sendRedirect(String.format(AceOAuthConstant.IMPLICIT_REDIRECT_URL, clientDetails.getWebServerRedirectUri(), aceOAuth2AccessToken.getAccessToken(), aceAuthorizeParams.state));
    ...
    public AceOAuth2AccessToken token(String code, String tenantId) {
        final AceOAuth2AccessToken aceOAuth2AccessToken = tokenRedisTemplate.opsForValue().get(String.format(AceOAuthConstant.CODE_PREFIX, code));
        if (aceOAuth2AccessToken != null) {
            tokenRedisTemplate.delete(String.format(AceOAuthConstant.CODE_PREFIX, code));
        }
        return aceOAuth2AccessToken;
    }
    ...
    public String login(HttpServletRequest request, HttpServletResponse response, HttpSession session, AceAuthorizeParams aceAuthorizeParams) {
        ...
        return "redirect:" + aceOAuthConfigProperties.getLoginPage() + "?responseType=" + aceAuthorizeParams.responseType + "&state=" + aceAuthorizeParams.state + "&clientId=" + aceAuthorizeParams.clientId;
    }
