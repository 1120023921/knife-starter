...
public class KnifeLogoutHandler implements LogoutHandler {
    ...
    public KnifeLogoutHandler(RedisTemplate<String, KnifeUser> userDatailRedisTemplate, RedisTemplate<String, KnifeOAuth2AccessToken> tokenRedisTemplate, ObjectMapper objectMapper) {
    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (header != null && header.startsWith("Bearer")) {
            //从token转用户
            final KnifeUser knifeUser = userDatailRedisTemplate.opsForValue().get(String.format(SecurityConstants.USER_DETAIL_PREFIX, header.split(" ")[1]));
            if (knifeUser != null) {
                //缓存删除用户
